...
const COMPLETED_REPORT_DATA = {
  ...
  suggestedFocus: '67% of students struggled with applying IAM in a real scenario. Consider a practical demo at the start of next session.',
  readinessStats: {
    onTrack: 14,
    needsReview: 10,
    atRisk: 6,
  ...
  studentsList: [
    { id: 1, name: 'Somchai Jaidee', initials: 'SJ', readiness: '76%', status: 'Partial', statusColor: 'bg-amber-100/70 text-amber-800', questions: 2 },
    { id: 2, name: 'Pim Nakorn', initials: 'PN', readiness: '92%', status: 'Met', statusColor: 'bg-emerald-100/70 text-emerald-800', questions: 0 },
    { id: 3, name: 'Kritsada Thongdee', initials: 'KT', readiness: '41%', status: 'Gap', statusColor: 'bg-rose-100/70 text-rose-800', questions: 4 },
  ],
};
...
export default function Page() {
  ...
  const isCompleted = currentSession.status === 'Completed' || !currentSession.isLive;
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 p-4 rounded-xl shadow-2xs">
                  <p className="text-[11px] font-semibold text-stone-400 mb-1">Avg readiness</p>
                  <p className="text-xl font-extrabold text-stone-900">{COMPLETED_REPORT_DATA.avgReadiness}</p>
                </div>

                <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 p-4 rounded-xl shadow-2xs">
                  <p className="text-[11px] font-semibold text-stone-400 mb-1">Students</p>
                  <p className="text-xl font-extrabold text-stone-900">{COMPLETED_REPORT_DATA.studentsCount}</p>
                </div>

                <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 p-4 rounded-xl shadow-2xs">
                  <p className="text-[11px] font-semibold text-stone-400 mb-1">Caught up</p>
                  <p className="text-xl font-extrabold text-stone-900">{COMPLETED_REPORT_DATA.caughtUpCount}</p>
                  <p className="text-[10px] text-stone-400 font-medium mt-0.5">{COMPLETED_REPORT_DATA.caughtUpPercent} of class</p>
                </div>

                <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 p-4 rounded-xl shadow-2xs">
                  <p className="text-[11px] font-semibold text-stone-400 mb-1">Weakest criterion</p>
                  <p className="text-xl font-extrabold text-stone-900">{COMPLETED_REPORT_DATA.weakestCriterion}</p>
                  <p className="text-[10px] text-stone-400 font-medium mt-0.5">{COMPLETED_REPORT_DATA.weakestDesc}</p>
                </div>
              </div>
              ...
              <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 rounded-xl p-4 shadow-2xs flex items-start gap-3">
                ...
                <div className="text-left">
                  <h4 className="text-xs font-bold text-stone-800">Suggested focus</h4>
              ...
              <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 rounded-xl p-6 shadow-2xs">
                <h3 className="text-xs font-bold text-stone-800 mb-4">Class readiness</h3>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-8">
                  ...
                  <div className="relative w-36 h-36 shrink-0 flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                      ...
                      <span className="text-[10px] font-medium text-stone-400 mt-0.5">on track</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-xs font-medium">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-stone-900">{COMPLETED_REPORT_DATA.readinessStats.onTrack}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
                      <span className="text-stone-600">Needs review</span>
                      <span className="font-bold text-stone-900">{COMPLETED_REPORT_DATA.readinessStats.needsReview}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-rose-500" />
                      <span className="text-stone-600">At risk</span>
                      <span className="font-bold text-stone-900">{COMPLETED_REPORT_DATA.readinessStats.atRisk}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/90 backdrop-blur-xs border border-stone-200/60 rounded-xl p-5 shadow-2xs">
                <h3 className="text-xs font-bold text-stone-800">Common issues this session</h3>
              </div>

              {/* Students Table */}
              <div className="pt-2">
                <h3 className="text-xs font-bold text-stone-800 mb-3">Students</h3>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-stone-100 text-stone-400 font-medium text-[11px]">
                        ...
                        <th className="py-3 px-4 text-right sm:text-left">During-class questions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100">
                      {COMPLETED_REPORT_DATA.studentsList.map((student) => (
                        <tr key={student.id} className="hover:bg-stone-50/50 transition-colors">
                          ...
                          <td className="py-3.5 px-4 font-semibold text-stone-700">{student.readiness}</td>
                          <td className="py-3.5 px-4">
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${student.statusColor}`}>
                              {student.status}
                            </span>
                          </td>
